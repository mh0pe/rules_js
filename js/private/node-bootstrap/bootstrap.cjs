...
if (JS_BINARY__CHDIR && isMainThread) {
    ...
    try {
        ...
        // A process that inherited NODE_OPTIONS from an already-running js_binary process (for
        // example a worker pool that forks using process.execPath, which we override above to
        // the node wrapper) may already be in the target directory, since cwd is inherited
        // across forks; chdir()ing again relative to it would incorrectly try to go one level
        // deeper. If we're already there, ignore the error; otherwise it's a real problem.
        const cwd = process.cwd()
        const alreadyThere =
            cwd === JS_BINARY__CHDIR || cwd.endsWith(path.sep + JS_BINARY__CHDIR)
        if (e.code !== 'ENOENT' || !alreadyThere) {
            throw e
        }
    }
}
