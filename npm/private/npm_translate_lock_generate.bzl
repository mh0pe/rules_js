...
# buildifier: disable=function-docstring
def generate_repository_files(attr, state, npm_imports):
    # empty line after bzl docstring since buildifier expects this if this file is vendored in
    generated_by_prefix = "\"\"\"@generated by npm_translate_lock(name = \"{}\", dependency_graph = \"{}\")\"\"\"\n".format(helpers.to_apparent_repo_name(attr.name), str(state.lock_label()))
...
""",
    ...
    for package_key, package_info in packages.items():
        ...
        if resolution.get("type", None) in ["directory", "virtual-directory"]:
...
{validation_call}""".format(
            defs_bzl_file = "@{}//:{}".format(attr.name, _DEFS_BZL_FILENAME),
