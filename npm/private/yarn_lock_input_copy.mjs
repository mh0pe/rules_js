...
export async function copyVerifiedInput(source, destination, hooks = {}) {
    ...
    if (sourceStat.isSymbolicLink() || !sourceStat.isFile()) {
        throw new Error(
            `binary_data source is not a regular file: ${sourcePath}`
        )
    }
    ...
    try {
        ...
        if (!existing.isSymbolicLink()) {
            throw new Error(
                `binary_data destination already exists: ${destinationPath}`
            )
        }
    ...
    try {
        ...
        if (hooks.afterCopy)
            await hooks.afterCopy({ sourcePath, temporaryPath })
        ...
        if (!destinationStat.isFile() || destinationStat.isSymbolicLink()) {
            throw new Error(
                `binary_data destination is not a regular copied file: ${destinationPath}`
            )
        }
...
async function main() {
    ...
    if (!source || !destination || process.argv.length !== 4) {
        throw new Error(
            'usage: yarn_lock_input_copy.mjs <source> <destination>'
        )
    }
