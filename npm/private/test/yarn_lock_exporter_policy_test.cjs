...
const rejectPatchLocator = ({
  ...
  ...locatorPolicyPaths,
  parseRange: currentReference => currentReference === reference
    ? {
        protocol: "patch:",
        source: "source-locator",
        selector,
        params: parentReference === null ? null : {locator: "parent-locator"},
      }
  ...
  parseLocator: value => ({
    reference: value === "source-locator" ? sourceReference : parentReference,
  }),
  toPortablePath: identityPortablePath,
});
...
const configuration = (globalValue, explicitlyConfigured = false) => ({
  ...
  sources: new Map(explicitlyConfigured ? [["httpsProxy", ".yarnrc.yml"]] : []),
});

assert.equal(classicHttpsProxyIsConfigured(configuration(null), null), false);
assert.equal(classicHttpsProxyIsConfigured(configuration(null, true), null), true);
