...
const classicRetryDelay = (error, attempt, networkTuning) => {
    if (attempt > networkTuning.retries) return null
    ...
    if (
        ...
        const retryAfter = classicRetryAfterMilliseconds(
            error.classicRetryAfter
        )
...
const rejectUnsafeYarnLocatorBeforeFetch = ({
    ...
    if (range.protocol !== 'patch:') return null
    ...
    const parentLocator =
        typeof range.params?.locator === 'string'
            ? parseLocator(range.params.locator)
            : null
    const parentLocalPath = recurse(parentLocator)

    for (const rawPath of range.selector ? range.selector.split(/&/) : []) {
        const path = toPortablePath(rawPath.slice(rawPath.lastIndexOf('!') + 1))
        if (/^builtin<[^>]+>$/.test(path)) continue
        ...
        if (path.startsWith('~/')) {
            containedPath(projectRoot, path.slice(2), 'Yarn project patch path')
            continue
        }
        if (parentLocator === null) {
            throw new Error(
        ...
        if (parentLocalPath !== null) {
            containedPath(parentLocalPath, path, 'Yarn user patch path')
        }
    }
    return null
}
...
module.exports = {
    ...
    factory: (require) => {
        ...
        const sortedObject = (entries) =>
            Object.fromEntries(sortEntries(entries))
        const jsonValue = (value) => {
            ...
            if (value instanceof Set) return [...value].map(jsonValue).sort()
            ...
            if (value instanceof Map) {
                for (const [key, item] of value) {
                    if (SENSITIVE_KEYS.has(String(key)))
                        throw new Error(
                            `${path} contains unsupported Yarn authentication key ${String(
                                key
                            )}`
                        )
                ...
                for (const [key, item] of Object.entries(value)) {
                    if (SENSITIVE_KEYS.has(key))
                        throw new Error(
                            `${path} contains unsupported Yarn authentication key ${key}`
                        )
        ...
        const validateClassicResolvedUrl = (value, path, requireLegacyHash) => {
            ...
            if (parsed.username || parsed.password || parsed.search)
                throw new Error(
                    `${path} cannot contain credentials or query parameters`
                )
            ...
            if (requireLegacyHash) {
                ...
                throw new Error(
                    `${path} redirect targets cannot contain fragments`
                )
            }
        ...
        const parseClassicIntegrity = (value, path) => {
            ...
            if (raw.trim() !== raw)
                throw new Error(
                    `${path} cannot contain leading or trailing whitespace`
                )
            const parsed = raw.split(/\s+/).map((token) => {
                const match =
                    /^(sha1|sha256|sha384|sha512)-([A-Za-z0-9+/]+={0,2})$/.exec(
                        token
                    )
                if (!match)
                    throw new Error(
                        `${path} contains a malformed or unsupported SRI token`
                    )
                ...
                if (
                    ...
                    throw new Error(
                        `${path} contains a noncanonical ${algorithm} digest`
                    )
                }
            ...
            const strongestPriority = Math.max(
                ...parsed.map((item) =>
                    CLASSIC_SRI_PRIORITY.get(item.algorithm)
                )
            )
        ...
        const verifyClassicArchive = (bytes, record) => {
            const actualLegacySha1 = createHash('sha1')
                .update(bytes)
                .digest('hex')
            ...
            const matchingIntegrity = record.strongestIntegrity
                .filter(
                    (item) =>
                        createHash(item.algorithm)
                            .update(bytes)
                            .digest('base64') === item.encoded
                )
                .sort((left, right) =>
        ...
        const classicNetworkOptions = async (configuration, url) => {
            const settings = httpUtils.getNetworkSettings(url, {
                configuration,
            })
            if (settings.enableNetwork === false) {
                throw new Error(
                    `Yarn network policy disables Classic archive fetch for ${url}`
                )
            }
            const proxyConfigured = classicHttpsProxyIsConfigured(
                configuration,
                settings.httpsProxy
            )
            return {
        ...
        const classicNetworkTuning = (configuration) => ({
            ...
            retries: classicNetworkInteger(configuration, 'httpRetry', 0),
            timeout: classicNetworkInteger(configuration, 'httpTimeout', 1),
        })
        ...
        const proxyForClassicTarget = (target, networkOptions) => {
            ...
            if (!raw) {
                if (networkOptions.proxyConfigured) return null
                if (noProxyMatches(target)) return null
                raw =
        ...
        const connectClassicProxyTunnel = (
            ...
            new Promise((accept, reject) => {
                const targetHostname = hostnameWithoutIpv6Brackets(
                    target.hostname
                )
                const targetAuthority =
                    `${
                        isIP(targetHostname) === 6
                            ? `[${targetHostname}]`
                            : targetHostname
                    }:` + `${target.port || '443'}`
                ...
                if (proxy.username || proxy.password) {
                    ...
                    try {
                        ...
                        return reject(
                            new Error(
                                'Proxy credentials are not valid percent-encoding'
                            )
                        )
                    }
                ...
                request.setTimeout(networkTuning.timeout, () =>
                    request.destroy(
                        classicTimeoutError(
                            `Proxy CONNECT timed out for ${display}`
                        )
                    )
                )
                request.once('connect', (response, socket, head) => {
                    ...
                    if (head.length !== 0) {
                        ...
                        return reject(
                            new Error(
                                `Proxy CONNECT for ${display} returned unexpected bytes`
                            )
                        )
                    }
                    const tlsSocket = tlsConnect({
                        ...tlsOptions,
                        servername: isIP(targetHostname)
                            ? undefined
                            : targetHostname,
                        socket,
                    })
                    tlsSocket.setTimeout(networkTuning.timeout, () =>
                        tlsSocket.destroy(
                            classicTimeoutError(
                                `Proxy TLS handshake timed out for ${display}`
                            )
                        )
                    )
        ...
        const requestClassicTarget = async (
            ...
            const networkOptions = await classicNetworkOptions(
                configuration,
                url
            )
        ...
        const fetchClassicArchive = async (
            ...
            const fetchOnce = (url, redirectCount) =>
                new Promise((accept, reject) => {
                    let request
                    const receive = (response) => {
                        const finish = async () => {
                            ...
                            if (status >= 300 && status < 400) {
                                ...
                                if (!location)
                                    throw new Error(
                                        `Redirect for ${display} has no Location header`
                                    )
                                if (redirectCount >= CLASSIC_MAX_REDIRECTS) {
                                    throw new Error(
                                        `Too many redirects while fetching ${display}`
                                    )
                                }
                                let redirected
                                try {
                                    ...
                                    throw new Error(
                                        `Invalid redirect URL while fetching ${display}`
                                    )
                                }
                            ...
                            if (
                                ...
                                (!/^(?:0|[1-9][0-9]*)$/.test(contentLength) ||
                                    Number(contentLength) >
                                        CLASSIC_MAX_ARCHIVE_BYTES)
                            ) {
                                response.destroy()
                                throw new Error(
                                    `Classic archive for ${display} exceeds the size limit`
                                )
                            }
                            return new Promise((acceptBody, rejectBody) => {
                                ...
                                response.on('data', (chunk) => {
                                    ...
                                    if (
                                        byteLength > CLASSIC_MAX_ARCHIVE_BYTES
                                    ) {
                                        response.destroy(
                                            new Error(
                                                `Classic archive for ${display} exceeds the size limit`
                    ...
                    requestClassicTarget(
                        ...
                        request.setTimeout(networkTuning.timeout, () =>
                            request.destroy(
                                classicTimeoutError(
                                    `HTTPS fetch timed out for ${display}`
                                )
                            )
                        )
        ...
        const tarString = (buffer, start, length, path) => {
            ...
            if (nul !== -1 && field.subarray(nul).some((byte) => byte !== 0))
                throw new Error(
                    `${path} contains bytes after its NUL terminator`
                )
            try {
        ...
        const safeTarPath = (value, path) => {
            if (
                ...
                throw new Error(
                    `${path} is not a safe canonical relative tar path`
                )
            }
            return value
        }
        ...
        const parsePaxPath = (bytes, path) => {
            ...
            while (offset < bytes.length) {
                ...
                if (equals <= 0)
                    throw new Error(
                        `${path} has a malformed PAX key/value record`
                    )
                const key = record.subarray(0, equals).toString('ascii')
                if (key !== 'path')
                    throw new Error(
                        `${path} contains unsupported PAX key ${JSON.stringify(
                            key
                        )}`
                    )
                if (paxPath !== null)
                    throw new Error(
                        `${path} contains duplicate PAX path records`
                    )
        ...
        const inspectClassicTarball = (archiveBytes, record) => {
            if (archiveBytes[0] !== 0x1f || archiveBytes[1] !== 0x8b)
                throw new Error(
                    `Classic archive for ${record.display} is not gzip`
                )
            let tar
            try {
            ...
            if (tar.length === 0 || tar.length % 512 !== 0)
                throw new Error(
                    `Classic tar for ${record.display} has an invalid length`
                )
            ...
            for (let offset = 0; offset < tar.length; ) {
                ...
                if (header.every((byte) => byte === 0)) {
                    if (
                        ...
                        throw new Error(
                            `Classic tar for ${record.display} has a malformed terminator`
                        )
                    }
                    sawEnd = true
                    break
                ...
                if (actualChecksum !== expectedChecksum)
                    throw new Error(
                        `Classic tar for ${record.display} has a bad header checksum`
                    )
                ...
                if (dataEnd > tar.length || nextOffset > tar.length)
                    throw new Error(
                        `Classic tar for ${record.display} has a truncated entry`
                    )
                ...
                if (type === 'x') {
                    if (pendingPath !== null)
                        throw new Error(
                            `Classic tar for ${record.display} stacks path extensions`
                        )
                ...
                if (paths.has(entryPath))
                    throw new Error(
                        `Classic tar contains duplicate path ${entryPath}`
                    )
                paths.add(entryPath)
                if (type !== '\0' && type !== '0' && type !== '5') {
                    throw new Error(
                        `Classic tar contains unsupported entry type ${JSON.stringify(
                            type
                        )} ` + `at ${entryPath}`
                    )
                }
                if (type === '5' && size !== 0)
                    throw new Error(
                        `Classic tar directory ${entryPath} has nonzero content`
                    )
                if (
                    (type === '\0' || type === '0') &&
                    ...
                    if (size > 5 * 1024 * 1024)
                        throw new Error(
                            `package/package.json for ${record.display} is too large`
                        )
                    packageJsonBytes = Buffer.from(data)
                }
                if (
                    (type === '\0' || type === '0') &&
                    entryPath === 'package/binding.gyp'
                )
                    hasBindingGyp = true
                offset = nextOffset
            }
            if (!sawEnd)
                throw new Error(
                    `Classic tar for ${record.display} has no end marker`
                )
            if (pendingPath !== null)
                throw new Error(
                    `Classic tar for ${record.display} ends after a path extension`
                )
            if (packageJsonBytes === null)
                throw new Error(
                    `Classic tar for ${record.display} has no package/package.json`
                )
        ...
        const classicManifestBins = (value, packageName) => {
            ...
            return sortedObject(
                Object.entries(raw).map(([name, target]) => {
                    ...
                    if (
                        ...
                        throw new Error(
                            `package.json bin target is not a safe relative path: ${path}`
                        )
                    }
                    return [name, path]
                })
        ...
        const classicManifestConditions = (manifest) => {
            ...
            for (const [field, key] of [
                ...
                if (!Array.isArray(values) || values.length === 0)
                    throw new Error(
                        `package.json ${field} must be a non-empty string or list`
                    )
                const atoms = [
                    ...new Set(
                        values.map((raw, index) => {
                            const value = expectClassicString(
                                raw,
                                `package.json ${field}[${index}]`
                            )
                            const negative = value.startsWith('!')
                            const bare = negative ? value.slice(1) : value
                            if (!/^[a-z0-9_-]+$/.test(bare))
                                throw new Error(
                                    `package.json ${field} contains unsupported value ${value}`
                                )
        ...
        const inspectClassicManifest = (
            ...
            for (const selector of record.descriptors) {
                ...
                const targetName =
                    parsedRange.source == null
                        ? requestedName
                        : structUtils.stringifyIdent(
                              structUtils.parseIdent(parsedRange.source)
                          )
                if (targetName !== name) {
                    throw new Error(
                        `Classic selector ${structUtils.stringifyDescriptor(
                            selector
                        )} cannot ` +
            ...
            const manifestDependencies = sortedObject(
                Object.entries(manifestDependenciesWithOptional).filter(
                    ([dependencyName]) =>
            ...
            for (const lifecycleName of lifecycleNames) {
                if (
                    ...
                    throw new Error(
                        `package.json scripts.${lifecycleName} must be a string`
                    )
                }
            }
            const requiresBuild =
            ...
            if (manifest.peerDependenciesMeta !== undefined) {
                ...
                for (const [peerName, metadata] of Object.entries(rawMeta)) {
                    const peerMeta = expectClassicObject(
                        metadata,
                        `package.json peerDependenciesMeta[${JSON.stringify(
                            peerName
                        )}]`
                    )
                    const unknown = Object.keys(peerMeta).filter(
                        (key) => key !== 'optional'
                    )
                    if (unknown.length > 0) {
                        throw new Error(
                    ...
                    if (
                        peerMeta.optional !== undefined &&
                        typeof peerMeta.optional !== 'boolean'
                    ) {
                        throw new Error(
                            `package.json peerDependenciesMeta[${JSON.stringify(
                                peerName
                    ...
                    if (peerMeta.optional !== undefined)
                        peerDependenciesMeta[peerName] = {
                            optional: peerMeta.optional,
                        }
                }
            }
            ...
            return {
                ...
                name,
                peerDependencies,
                peerDependenciesMeta: sortedObject(
        ...
        const classicDescriptor = (configuration, name, range, path) => {
            ...
            try {
                descriptor = structUtils.makeDescriptor(
                    structUtils.parseIdent(
                        expectClassicString(name, `${path} name`)
                    ),
                ...
                throw new Error(
                    `${path} is not a valid Yarn descriptor: ${error.message}`
                )
            }
        ...
        const parseClassicLock = (content, configuration) => {
            ...
            try {
                ...
                throw new Error(
                    `Classic yarn.lock is malformed: ${error.message}`
                )
            }
            parsed = expectClassicObject(parsed, 'Classic yarn.lock')
            const selectorToRecord = new Map()
            const records = []
            for (const selectorList of Object.keys(parsed).sort(
                compareCodeUnits
            )) {
                ...
                if (unknown.length > 0) {
                    throw new Error(
                        `Classic lock entry ${JSON.stringify(
                            selectorList
                ...
                if (
                    selectors.length === 0 ||
                    selectors.some(
                        (selector) => !selector || selector.trim() !== selector
                    )
                ) {
                    throw new Error(
                        `Classic lock entry has malformed selector list ${selectorList}`
                    )
                }
                const descriptors = selectors.map((selector) => {
                    ...
                    try {
                        ...
                        throw new Error(
                            `Classic selector ${JSON.stringify(
                                selector
                ...
                for (const descriptor of descriptors) {
                    const previous = selectorToRecord.get(
                        descriptor.descriptorHash
                    )
                    if (previous && previous !== record) {
                        throw new Error(
                            `Classic descriptor ${structUtils.stringifyDescriptor(
                                descriptor
        ...
        const classicReference = (project, target, requestedDescriptor) => {
            if (target.workspace) {
                return `link:${
                    ppath.relative(project.cwd, target.workspace.cwd) || '.'
                }`
            }
            const requestedName =
                structUtils.stringifyIdent(requestedDescriptor)
        ...
        const exportClassicGraph = async ({
            ...
            const enqueue = (target, category) => {
                if (target.record) queue.push([target.record, category])
            }
            ...
            const fetchNextRecord = async () => {
                while (nextRecord < orderedRecords.length) {
                    ...
                    record.archiveRelative = `archives/classic-${record.locatorHash.slice(
                        0,
                        32
                    )}.tgz`
                    ...
                    await writeFile(
                        resolve(
                            archiveDirectory,
                            record.archiveRelative.slice('archives/'.length)
                        ),
                        bytes
                    )
                }
            ...
            for (const record of orderedRecords) {
                ...
                const normalizedVersion = `${
                    manifest.version
                }_yarn_${record.locatorHash.slice(0, 32)}`
                const identity = {
                    ...
                    return [
                        ppath.relative(project.cwd, workspace.cwd) || '.',
                        {
                            ...
                            optional_dependencies:
                                sortedObject(optionalDependencies),
                            install_config: {
                                hoistingLimits:
                                    manifest.installConfig?.hoistingLimits ??
                                    null,
                                selfReferences:
                                    manifest.installConfig?.selfReferences ??
                                    null,
                            },
                        },
                    ]
            ...
            const packages = orderedRecords.map((record) => {
                ...
                const optionalDependencyEntries =
                    record.optionalDependencyTargets.map(
                        ([descriptor, target]) => [
                            structUtils.stringifyIdent(descriptor),
                            classicReference(project, target, descriptor),
                        ]
        ...
        const writeVerifiedGraph = async (
            graph,
            outputPath,
            expectedGraphSha256
        ) => {
        ...
        const configurationValue = (configuration, key) => {
            if (!configuration.settings.has(key)) return null
        ...
        const configurationMetadata = (configuration) => {
            ...
            const metadata = sortedObject(
                keys.map((key) => [key, configurationValue(configuration, key)])
            )
            ...
            metadata.registryPolicy = {
                npmAlwaysAuth: configurationValue(
                    configuration,
                    'npmAlwaysAuth'
                ),
                ...
                registries: [...registries]
                    .map(([registry, settings]) => ({
                        url: safeRegistryValueUrl(
                            registry,
                            'npmRegistries key'
                        ),
                ...
                scopes: sortedObject(
                    [...scopes].map(([scope, settings]) => [
                        scope,
                        safeRegistrySettings(
                            settings,
                            `npmScopes[${JSON.stringify(scope)}]`
                        ),
                    ])
                ),
            }
            ...
            for (const pattern of pnpIgnorePatterns) {
                ...
                if (
                    ...
                    (configuration.projectCwd &&
                        pattern.includes(configuration.projectCwd)) ||
            ...
            metadata.enableScripts = configurationValue(
                configuration,
                'enableScripts'
            )
        ...
        const dependencyMetaFor = (metaMap, descriptor) => {
            ...
            if (!(byRange instanceof Map)) return byRange
            return byRange.get(descriptor.range) || byRange.get(null)
        }
        ...
        const dependencyMetadata = (metaMap) => {
            ...
            return sortedObject(
                [...metaMap].map(([ident, ranges]) => {
                    if (!(ranges instanceof Map))
                        throw new Error(
                            `dependenciesMeta[${String(
                                ident
                            )}] must be a range map`
                        )
                    return [
        ...
        const validatePackageManager = (project) => {
            ...
            const declarations = [
                ...
                [
                    ...
                    rawManifest.devEngines?.packageManager,
                ],
            ]
            for (const [field, raw] of declarations) {
                ...
                if (!parsed || parsed.name !== 'yarn') {
                    throw new Error(
                        `${field} must select Yarn when using a yarn.lock; found ${JSON.stringify(
                            raw
                        )}`
                    )
                }
        ...
        const resolvedReference = (project, descriptor, identities) => {
            ...
            if (!locatorHash) {
                throw new Error(
                    `No stored Yarn resolution for ${structUtils.stringifyDescriptor(
                        descriptor
                    )}`
                )
            }
            ...
            const workspaceLocator = structUtils.isVirtualLocator(
                resolvedLocator
            )
            ...
            if (!identity)
                throw new Error(
                    `No normalized identity for locator hash ${locatorHash}`
                )
            const requestedName = structUtils.stringifyIdent(descriptor)
            return requestedName === identity.name
                ? identity.version
        ...
        const validateWorkspaceManifestsAgainstLock = (project) => {
            const discoveredWorkspaceHashes = new Set(
                project.workspaces.map(
                    (workspace) => workspace.anchoredLocator.locatorHash
                )
            )
            for (const lockedPackage of project.originalPackages.values()) {
                if (
                    !structUtils.isVirtualLocator(lockedPackage) &&
                    structUtils.parseRange(lockedPackage.reference).protocol ===
                        'workspace:' &&
            ...
            for (const workspace of project.workspaces) {
                ...
                if (!lockedPackage)
                    throw new Error(
                        `Yarn lockfile is missing workspace ${workspacePath}`
                    )

                for (const descriptor of workspace.manifest.dependencies.values()) {
                    if (
                        workspace.manifest.devDependencies.has(
                ...
                for (const [identHash, descriptor] of declared) {
                    ...
                    const lockedDescriptor =
                        lockedPackage.dependencies.get(identHash)
                    if (!lockedDescriptor) {
                        throw new Error(
                    ...
                    if (
                        normalized.descriptorHash !==
                        lockedDescriptor.descriptorHash
                    ) {
                        throw new Error(
        ...
        class FrozenPackageResolver {
            ...
            supportsDescriptor(descriptor) {
                const locatorHash = this.resolutions.get(
                    descriptor.descriptorHash
                )
                return (
                    locatorHash !== undefined && this.packages.has(locatorHash)
                )
            }
            ...
            async getSatisfying(descriptor, dependencies, locators) {
                const [expected] = await this.getCandidates(
                    descriptor,
                    dependencies
                )
                return {
                    locators: locators.filter(
                        (locator) =>
                            locator.locatorHash === expected.locatorHash
                    ),
                    sorted: false,
                }
            }
            ...
            async resolve(locator) {
                ...
                if (!pkg) {
                    throw new Error(
                        `Locator is not frozen in yarn.lock: ${safeLocatorDisplay(
                            locator
                        )}`
                    )
                }
        ...
        class ClosedResolver {
            ...
            supportsDescriptor(descriptor, opts) {
                return this.resolvers.some((candidate) =>
                    candidate.supportsDescriptor(descriptor, opts)
                )
            }
            ...
            bindDescriptor(descriptor, fromLocator, opts) {
                return this.descriptorResolver(descriptor, opts).bindDescriptor(
                    descriptor,
                    fromLocator,
                    opts
                )
            ...
            getCandidates(descriptor, dependencies, opts) {
                return this.descriptorResolver(descriptor, opts).getCandidates(
                    descriptor,
                    dependencies,
                    opts
                )
            ...
            getSatisfying(descriptor, dependencies, locators, opts) {
                return this.descriptorResolver(descriptor, opts).getSatisfying(
                    descriptor,
                    dependencies,
                    locators,
                    opts
            ...
            resolve(locator, opts) {
                return this.locatorResolver(locator, opts).resolve(
                    locator,
                    opts
                )
            }
        ...
        const closeBerryResolution = (project) => {
            ...
            for (const [descriptorHash, locatorHash] of [
                ...project.storedResolutions,
            ]) {
                if (!workspaceHashes.has(locatorHash)) continue
        ...
        const packageSignature = (
            ...
            const locator =
                devirtualize && structUtils.isVirtualLocator(pkg)
                    ? structUtils.devirtualizeLocator(pkg)
                    : pkg
            const descriptorHash = (descriptor) => {
                const normalized =
                    devirtualize && structUtils.isVirtualDescriptor(descriptor)
                        ? structUtils.devirtualizeDescriptor(descriptor)
                        : descriptor
                return (
                    resolutions?.get(normalized.descriptorHash) ||
                    normalized.descriptorHash
                )
            }
            return {
                ...
                dependencies: sortedObject(
                    [...pkg.dependencies].map(([identHash, descriptor]) => [
                        identHash,
                ...
                peerDependencies: sortedObject(
                    [...pkg.peerDependencies].map(([identHash, descriptor]) => [
                        identHash,
                        descriptorHash(descriptor),
                    ])
                ),
                ...
                preferUnplugged:
                    typeof pkg.preferUnplugged === 'boolean'
                        ? pkg.preferUnplugged
                        : null,
                version: pkg.version,
            }
        ...
        const validateResolvedPackagesAgainstLock = async (
            ...
            const allExtensions =
                await project.configuration.getPackageExtensions()
            ...
            for (const [locatorHash, frozen] of frozenPackages) {
                ...
                if (!workspaceForLocator(project, frozen)) {
                    const expectedBuiltin =
                        await prepareWithPinnedCompatibility(
                            project,
                            frozen,
                            resolver,
                            report,
                    ...
                    const expectedBuiltinSignature =
                        packageSignature(expectedBuiltin)
                    if (
                        Object.keys(expectedBuiltinSignature).some(
                            (field) =>
                                JSON.stringify(rawSignature[field]) !==
            ...
            for (const descriptorHash of [...resolutionHashes].sort()) {
                const frozenLocatorHash =
                    frozenNonWorkspaceResolutions.get(descriptorHash)
                const resolvedLocatorHash =
                    resolvedNonWorkspaceResolutions.get(descriptorHash)
                if (frozenLocatorHash !== resolvedLocatorHash) {
                    ...
                    if (
                        ...
                        const devirtualizedDescriptor =
                            structUtils.devirtualizeDescriptor(descriptor)
                        const frozenPhysicalLocatorHash = frozenResolutions.get(
                            devirtualizedDescriptor.descriptorHash
                        )
                        const resolvedLocator =
                            project.storedPackages.get(resolvedLocatorHash)
                        const resolvedPhysicalLocator =
                            resolvedLocator &&
                            structUtils.isVirtualLocator(resolvedLocator)
                                ? structUtils.devirtualizeLocator(
                                      resolvedLocator
                                  )
                                : resolvedLocator
                        generatedVirtualResolution =
                            frozenPhysicalLocatorHash !== undefined &&
                            resolvedPhysicalLocator?.locatorHash ===
                                frozenPhysicalLocatorHash
                    }
                    if (generatedVirtualResolution) continue
                    throw new Error(
            ...
            const frozenNonWorkspacePackages = new Map(
                [...frozenPackages].filter(
                    ([, pkg]) => !workspaceForLocator(project, pkg)
                )
            )
            const resolvedNonWorkspacePackages = new Map(
                [...project.storedPackages].filter(
                    ([, pkg]) => !workspaceForLocator(project, pkg)
                )
            )
            ...
            for (const locatorHash of [...locatorHashes].sort()) {
                ...
                if (!frozen || !resolved) {
                    ...
                    if (
                        ...
                        const physical =
                            structUtils.devirtualizeLocator(resolved)
            ...
            for (const [locatorHash, frozen] of frozenPackages) {
                ...
                const expectedSignature = packageSignature(
                    expected,
                    signatureOptions
                )
                ...
                const differingFields = signatureFields.filter(
                    (field) =>
                        JSON.stringify(actualSignature[field]) !==
                        JSON.stringify(expectedSignature[field])
                )
                if (differingFields.length > 0) {
                    throw new Error(
                        `Resolved package metadata differs from yarn.lock after applying only ` +
                            `pinned Yarn built-in compatibility: ${safeLocatorDisplay(
                                pkg
                            )} ` +
        ...
        const boundWorkspaceDescriptor = (project, workspace, descriptor) => {
            const bound = workspace.anchoredPackage.dependencies.get(
                descriptor.identHash
            )
            if (!bound) {
                throw new Error(
                    `Resolved workspace ${
                        ...
                        `does not contain ${structUtils.stringifyDescriptor(
                            descriptor
                        )}`
                )
            }
        ...
        const computeReachability = (project) => {
            ...
            const queue = []
            const enqueue = (descriptor, category) =>
                queue.push([descriptor, category])
        ...
        const stableArchiveName = (locator) => {
            const slug = structUtils
                .slugifyLocator(locator)
        ...
        const safeLocatorDisplay = (locator) => {
            ...
            return (
                ...
                `${
                    parsedRange.protocol || 'unknown:'
                }<redacted>#${locator.locatorHash.slice(0, 12)}`
            )
        }
        ...
        const fetchAllAccessible = async (
            ...
            const accessible =
                project.accessibleLocators instanceof Set
            ...
            for (const locatorHash of [...accessible].sort()) {
                ...
                if (!pkg)
                    throw new Error(
                        `Accessible Yarn locator ${locatorHash} has no stored package`
                    )
                const locator = structUtils.convertPackageToLocator(pkg)
                const physical = structUtils.isVirtualLocator(locator)
                    ? structUtils.devirtualizeLocator(locator)
                    : locator
                if (
                    workspaceForLocator(project, physical) ||
                    physicalPackages.has(physical.locatorHash)
                )
                    continue
                ...
                if (
                    ...
                    LOCAL_PROTOCOLS.has(parsedRange.protocol)
                ) {
                    throw new Error(
                        `Non-workspace local package ${structUtils.stringifyLocator(
                            locator
                        )} ` +
                            `uses ${
                                parsedRange.protocol || 'a soft link'
                            }; declare it as a workspace input`
                    )
                }
                ...
                physicalPackages.set(physical.locatorHash, {
                    physical,
                    pkg:
                        project.storedPackages.get(physical.locatorHash) || pkg,
                })
            }

            const packages = [...physicalPackages.values()].sort(
                (left, right) =>
                    compareCodeUnits(
            ...
            const fetchNext = async () => {
                while (nextPackage < packages.length) {
                    ...
                    if (!expectedChecksum && !pkg.conditions) {
                        throw new Error(
                            `yarn.lock has no native checksum for ` +
                                `${safeLocatorDisplay(
                                    physical
                                )}; refusing to bless fetched bytes`
                        )
                    }
                    ...
                    try {
                        ...
                        const manifest = await Manifest.tryFind(
                            result.prefixPath,
                            {
                        ...
                        const hasLifecycleScript =
                            ...
                            ['preinstall', 'install', 'postinstall'].some(
                                (name) => manifest.scripts.has(name)
                            )
                        const hasBindingGyp =
                            await result.packageFs.existsPromise(
                                ppath.join(result.prefixPath, 'binding.gyp')
                            )
                        const dependencyMeta = project.getDependencyMeta(
                            physical,
                            pkg.version
                        )
                        ...
                        if (typeof dependencyMeta.built === 'boolean')
                            evidence.dependency_meta_built =
                                dependencyMeta.built
                        if (typeof dependencyMeta.unplugged === 'boolean')
                            evidence.dependency_meta_unplugged =
                                dependencyMeta.unplugged
            ...
            await Promise.all(
                Array.from({ length: Math.min(32, packages.length) }, () =>
                    fetchNext()
                )
            )
                ...
                if (
                    ...
                    throw new Error(
                        `Native Yarn graph export requires checksumBehavior: throw; ` +
                            `${configPath} selects ${JSON.stringify(
                                parsed.checksumBehavior
                            )}`
                    )
                }
                return 0
        ...
        class RulesJsExportCommand extends BaseCommand {
            ...
            expectedGraphSha256 = Option.String('--expected-graph-sha256', {
                required: true,
            })
            repositoryRoot = Option.String('--repository-root', {
                required: true,
            })
            sourceFormat = Option.String('--source-format', { required: true })
            sourcePackage = Option.String('--source-package', {
                required: true,
            })
            exporterYarnVersion = Option.String('--exporter-yarn-version', {
                required: true,
            })
            ...
            async execute() {
                if (this.sourceFormat !== 'auto')
                    throw new Error(
                        `source format must be auto, got ${this.sourceFormat}`
                    )
                ...
                const safeConfigurationMetadata =
                    configurationMetadata(configuration)
                const { project } = await Project.find(
                    configuration,
                    this.context.cwd
                )
                const repositoryRoot = npath.toPortablePath(
                    resolve(this.repositoryRoot)
                )
                const projectRelative = ppath.relative(
                    repositoryRoot,
                    project.cwd
                )
                if (
                ...
                const originalLock = await readFile(
                    npath.fromPortablePath(lockPath),
                    'utf8'
                )
                ...
                if (source.format === 'classic-v1') {
                    ...
                    const currentLock = await readFile(
                        npath.fromPortablePath(lockPath),
                        'utf8'
                    )
                    if (currentLock !== originalLock)
                        throw new Error(
                            'native Yarn graph export modified the copied source lockfile'
                        )
                    await writeVerifiedGraph(
                        graph,
                        outputPath,
                        this.expectedGraphSha256
                    )
                    return 0
                }
                ...
                const resolutionReport = await StreamReport.start(
                    ...
                    async (report) => {
                        ...
                        resolutionAdjustments =
                            await validateResolvedPackagesAgainstLock(
                                project,
                        ...
                        if (source.version === 8) {
                            ...
                            const originalCanonicalLock =
                                canonicalGeneratedLock(originalLock)
                            const resolvedPatchSources =
                                patchSourceLocatorHashes(project.storedPackages)
                            const lockMatches =
                                resolvedPatchSources.size === 0
                ...
                const importers = sortedObject(
                    project.workspaces.map((workspace) => {
                        ...
                        const optional = (descriptor) =>
                            isOptional(
                                effectivePackage.dependenciesMeta,
                                descriptor
                            )
                        ...
                        return [
                            ppath.relative(project.cwd, workspace.cwd) || '.',
                            {
                                ...
                                optional_dependencies: dependencyMap(
                                    ...
                                    identities,
                                    (descriptor) =>
                                        !dev(descriptor) && optional(descriptor)
                                ),
                                install_config: {
                                    hoistingLimits:
                ...
                const orderedPackages = [...accessible]
                    .map((locatorHash) =>
                        project.storedPackages.get(locatorHash)
                    )
                ...
                for (const pkg of orderedPackages) {
                        ...
                        packages.push([
                            identity.key,
                            {
                                ...
                                peer_dependencies: sortedObject(
                                    [...pkg.peerDependencies.values()].map(
                                        (descriptor) => [
                                            structUtils.stringifyIdent(
                                                descriptor
                                            ),
                    ...
                    const parsedRange = structUtils.parseRange(
                        locator.reference
                    )
                    if (
                        pkg.linkType === LinkType.SOFT ||
                    ...
                    const archiveLocator = structUtils.isVirtualLocator(locator)
                        ...
                        : locator
                    const yarnChecksum =
                        project.storedChecksums.get(
                            archiveLocator.locatorHash
                        ) ||
                    ...
                    const previousArchiveLocator =
                        archiveLocators.get(archiveRelative)
                    if (
                    ...
                    if (!archiveSha256) {
                        const sourcePath = cache.getLocatorPath(
                            archiveLocator,
                            yarnChecksum
                        )
                        if (!sourcePath || !xfs.existsSync(sourcePath)) {
                            throw new Error(
                                `Yarn cache archive missing for ${structUtils.stringifyLocator(
                                    archiveLocator
                                )}`
                            )
                        }
                        ...
                        await copyFile(
                            npath.fromPortablePath(sourcePath),
                            archivePath
                        )
        ...
        return {
            commands: [RulesJsInspectConfigCommand, RulesJsExportCommand],
        }
    },
}
